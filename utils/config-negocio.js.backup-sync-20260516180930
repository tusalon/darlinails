...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'f6552646-5c80-4482-bf88-392c79c62eaf'; // ID de Darly Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Darly Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '54118629';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'darlenyssolis.01@icloud.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Darly Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'darlinails';
};
...
console.log('✅ config-negocio.js listo para Darly Nails');
